...
def transform_dataset_for_training(input_filepath, output_filepath):
    ...
    df_new["timestamp"] = pd.to_datetime(df_orig["trans_date_trans_time"]).dt.strftime(
        "%Y-%m-%dT%H:%M:%S"
    )
...
if __name__ == "__main__":
    transform_dataset_for_training("fraudTest.csv", "fraudTest.csv")
    transform_dataset_for_training("fraudTrain.csv", "fraudTrain.csv")
