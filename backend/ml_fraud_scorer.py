"""ML fraud scorer: model decides flags/scores, heuristic engine explains them."""
...
def _apply_model_fallback_reasons(df: pd.DataFrame, threshold: float) -> pd.DataFrame:
    """When the model flags a row the heuristic did not explain, add plain language."""
    out = df.copy()
    breakdowns: List[str] = []
...
def ml_fraud_detection(
    ...
    *,
    ...
    """Flag with the trained model; explain with the heuristic reviewer copy."""
    if df.empty:
        return _empty_result(df)

    pipeline = get_pipeline(model_path or DEFAULT_MODEL_PATH)
    decisions = _model_decisions(pipeline, df)
    explained = simple_fraud_detection(df)

    result = explained.copy()
